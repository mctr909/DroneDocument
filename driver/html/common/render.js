...
class UniformVar {
	...
	drawModel(matM, group) {
		// uniformへ座標変換行列を登録
		this.#matMVP.mul(this.#matVP, matM);
	...
	applyCamera(matP, cam) {
		// ビュー×プロジェクション座標変換行列
		this.#matV.setView(cam.position, cam.eye, cam.upDirection, cam.rotation);
		this.#matVP.mul(matP, this.#matV);
		// uniformへ視線を登録
		this.#gl.uniform3fv(this.#eyeDirection, cam.eye);
	}
